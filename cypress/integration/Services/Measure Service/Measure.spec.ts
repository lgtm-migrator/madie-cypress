    ...
    it('Create New Measure, successful creation', () => {
        ...
        cy.getCookie('accessToken').then((accessToken) => {
            cy.request({
                ...
                body: {"measureName": measureName, "cqlLibraryName": CQLLibraryName, "model": model, "measureScoring": measureScoring, "measurementPeriodStart": "2022-01-01T05:00:00.000+0000",
...
describe('Measure Service: Create different Measure types', () => {
    ...
    afterEach('cleanup', () => {
        ...
        Utilities.deleteMeasure(measureName, newCQLLibraryName, measureScoring, measurementPeriodStart, measurementPeriodEnd)
...
describe('Measure Service: Update Delete Flag', () => {
    ...
    beforeEach('Set access token and create a new measure to run update against', () =>{
            ...
            cy.getCookie('accessToken').then((accessToken) => {
                cy.readFile('cypress/fixtures/measureId').should('exist').then((id) => {
                    cy.request({
                        ...
                        body: {"id": id, "measureName": newMeasureName, "cqlLibraryName": newCQLLibraryName, "model": model, "measureScoring": measureScoring, "measurementPeriodStart": "2022-01-01T05:00:00.000+0000",
                               "measurementPeriodEnd": "2023-01-01T05:00:00.000+0000", "active": false, "createdBy": defaultUser}
                    }).then((response) => {
                        expect(response.status).to.eql(200)
        ...
        it('Attempt to update / delete measure that does not belong to current user', () => {
            ...
            Utilities.deleteMeasure(newMeasureName, newCQLLibraryName, measureScoringU, measurementPeriodStart, measurementPeriodEnd)
        })
    ...
    it('Verify error message when the Measurement Period end date is after the start date', () => {
        ...
        cy.getCookie('accessToken').then((accessToken) => {
            cy.request({
                ...
                body: {"measureName": measureName, "cqlLibraryName": CQLLibraryName, "model": model, "measureScoring": measureScoring, "measurementPeriodStart": "2023-01-01T05:00:00.000+0000",
                       "measurementPeriodEnd": "2021-01-01T05:00:00.000+0000"}
            }).then((response) => {
                expect(response.status).to.eql(400)
